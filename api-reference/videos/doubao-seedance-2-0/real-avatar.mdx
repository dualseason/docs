...
<RequestExample>
  ...
  curl --request POST \
    --url https://claw.dualseason.com/v1/seedance2/real-avatar \
  ...
  const url = "https://claw.dualseason.com/v1/seedance2/real-avatar";
  const headers = {
    "Authorization": "Bearer <token>",
    "Content-Type": "application/json"
  };
...
素材提交后为异步审核任务，使用 [获取任务状态](/api-reference/tasks/status) 接口查询：
...
审核通过后，将 `asset://...` URL 直接传入 [Seedance 2.0 视频生成](/api-reference/videos/doubao-seedance-2-0/generation) 接口：
