---
title: '统一视频生成'
description: '通过同一个异步接口调用 Veo 3.1、Sora 2 和 Seedance 视频模型。'
api: 'POST /v1/video/generations'
---

使用统一接口提交文生视频或图生视频任务。Veo 3.1、Sora 2 和 Seedance 使用相同的提交与查询流程。
...
需要使用 OpenAI Videos API 路径时，也可以通过 `POST /v1/videos` 创建任务，并通过 `GET /v1/videos/{task_id}` 查询。该入口使用 `multipart/form-data`，与本页的统一 JSON 请求格式不同。

<Card title="Grok Imagine 1.5 视频生成" icon="video" href="/api-reference/videos/grok-imagine-video-1.5">
  查看 Grok 的 multipart 参数、任务查询、视频下载、参考图限制和完整示例。
</Card>
