...
<RequestExample>
  ...
  curl --request POST \
    --url https://claw.dualseason.com/v1/uploads/images \
  ...
  with open('image.jpg', 'rb') as f:
      response = requests.post(
          "https://claw.dualseason.com/v1/uploads/images",
          headers={
  ...
  # 使用上传的图片进行生成
  response = requests.post(
      "https://claw.dualseason.com/v1/images/generations",
      headers={
          "Authorization": "Bearer <token>",
  ...
  const uploadResponse = await fetch('https://claw.dualseason.com/v1/uploads/images', {
    method: 'POST',
    headers: {
      'Authorization': 'Bearer <token>'
    },
  ...
  // 使用上传的图片进行生成
  const genResponse = await fetch('https://claw.dualseason.com/v1/images/generations', {
    method: 'POST',
    headers: {
      'Authorization': 'Bearer <token>',
      'Content-Type': 'application/json'
