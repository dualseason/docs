...
统一的 OpenAI 兼容端点，支持 GPT-5、Claude 和 Gemini。只需将 Base URL 更改为 `https://claw.dualseason.com/v1`，即可在几分钟内从 OpenAI 迁移——保留现有 SDK，无需重写代码。多供应商路由确保低延迟和高可用性。透明定价、企业 SLA 和全球 CDN 加速。
...
client = OpenAI(
    base_url="https://claw.dualseason.com/v1",
...
<Steps>
  <Step title="获取 API Key">
    注册 APIMart 并从<a href="https://claw.dualseason.com/console/token" rel="nofollow" target="_blank">控制台</a>获取 API Key。
  </Step>
...
<AccordionGroup>
  <Accordion title="如何从 OpenAI 迁移？">
    只需将 Base URL 更改为 `https://claw.dualseason.com/v1` 并使用您的 APIMart API Key。保留现有 SDK——无需重写代码。迁移通常在 5 分钟内完成。
  </Accordion>
