...
PATH 中应包含 `%NVM_HOME%`、`%NVM_SYMLINK%` 和 `%APPDATA%\npm`。如果 PATH 中同时存在旧版 Node.js 路径，会导致版本切换不生效。
...
💡 如果 `curl` 下载失败，可以用浏览器打开 https://github.com/nvm-sh/nvm/releases 下载 `install.sh`，然后执行 `bash ~/Downloads/install.sh`。
...
```powershell
echo $env:PATH -split ';' | Select-String -Pattern "node"
```
...
```
nvm: No available version of node found for xxx
```
