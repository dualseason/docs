...
description: "详细指导如何在 Claude Code CLI 中配置和使用 APIMart API 服务，通过简单的配置即可在终端中调用多种 AI 模型进行辅助编程。"
---
...
1. **已获取 APIMart API 密钥**
   登录 [APIMart 控制台](https://claw.dualseason.com/console/token) 获取您的 API 密钥（以 `sk-` 开头）

<Note>**提示：** 如果还没有 APIMart 账户，请先在 [APIMart](https://claw.dualseason.com/console) 注册并获取 API 密钥。</Note>
...
{
  "env": {
    "ANTHROPIC_BASE_URL": "https://claw.dualseason.com",
...
| 参数                                         | 说明                                          |
| ------------------------------------------ | ------------------------------------------- |
| `ANTHROPIC_BASE_URL`                       | APIMart API 地址，固定为 `https://claw.dualseason.com` |
...
<Tabs>
  <Tab title="macOS (zsh)">
    ```bash theme={null}
    echo 'export ANTHROPIC_BASE_URL="https://claw.dualseason.com"' >> ~/.zshrc
  ...
  <Tab title="macOS / Linux (bash)">
    ```bash theme={null}
    echo 'export ANTHROPIC_BASE_URL="https://claw.dualseason.com"' >> ~/.bashrc
  ...
  <Tab title="Windows">
    ...
    **方式 B：PowerShell 命令设置**
    ...
    ```powershell theme={null}
    [System.Environment]::SetEnvironmentVariable('ANTHROPIC_BASE_URL', 'https://claw.dualseason.com', 'User')
...
<Tabs>
  <Tab title="macOS / Linux">
    ...
    export ANTHROPIC_BASE_URL="https://claw.dualseason.com"
    claude
    ```
  </Tab>

  <Tab title="Windows (PowerShell)">
    `powershell $env:ANTHROPIC_API_KEY="sk-xxxxxxxxxxxx" $env:ANTHROPIC_MODEL="claude-sonnet-4-6-apimart" $env:ANTHROPIC_BASE_URL="https://claw.dualseason.com" claude `
  </Tab>
  ...
  <Tab title="Windows (CMD)">
    ...
    set ANTHROPIC_BASE_URL=https://claw.dualseason.com
    claude
    ```
  </Tab>
</Tabs>
...
| `403 Forbidden`    | 权限不足或密钥过期   | 前往 [控制台](https://claw.dualseason.com/console/token) 确认密钥状态 |

同时确保 `ANTHROPIC_BASE_URL` 设置为 `https://claw.dualseason.com`，而非 Anthropic 官方地址。
...
3. 如果使用了代理，确保代理设置允许访问 `claw.dualseason.com`
...
登录 [APIMart 控制台](https://claw.dualseason.com/console) 查看：
...
<Card title="开始使用 APIMart" icon="rocket" href="https://claw.dualseason.com/console">
