...
<Note>**重要提示：** APIMart 完全兼容 OpenAI API 格式，因此选择 "OpenAI Compatible" 即可。</Note>
...
| **Base URL**                  | `https://claw.dualseason.com/v1`            | APIMart API 基础地址  |
| **OpenAI Compatible API Key** | `sk-xxxxxxxxxxxx`                      | 您的 APIMart API 密钥 |
| **Model ID**                  | `gpt-5` 或 `claude-sonnet-4-5-20250929` | 输入要使用的模型 ID       |

<Frame>
  <img src="https://mintcdn.com/apicore/0c3FfTpBvR13Ca4x/images/integrations/cline/api-config.png?fit=max&auto=format&n=0c3FfTpBvR13Ca4x&q=85&s=a12d36a260af9e0cbb1eeb94a220cd4e" alt="配置 APIMart API" width="1920" height="1031" data-path="images/integrations/cline/api-config.png" />
</Frame>

<caption>*在 Settings 界面配置 API Provider、Base URL、API Key 和 Model ID*</caption>

<Note>
  **配置说明：** - **API Key**：必须是从 [APIMart 控制台](https://claw.dualseason.com/console/token) 获取的以 `sk-` 开头的密钥 - **Base URL**：固定为 `https://claw.dualseason.com/v1`，注意包含
...
2. **检查 Base URL**：
   * 必须是 `https://claw.dualseason.com/v1`
   * 注意包含 `/v1` 后缀

3. **验证密钥有效性**：
   * 在 [APIMart 控制台](https://claw.dualseason.com/console/token) 检查密钥状态
   * 确认账户有足够余额
...
4. **监控使用情况**：
   * 定期查看 [APIMart 控制台](https://claw.dualseason.com/console)
   * 了解 API 调用频率和费用
...
<Note>**即将推出：** 我们正在制作详细的视频教程，涵盖 Cline + APIMart 的各种使用场景和最佳实践。</Note>
