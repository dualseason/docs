...
| 模型 | 渠道 | 上下文 | 特点 | 适用场景 |
| --- | --- | --- | --- | --- |
| `gpt-5.5` | GPT 福利 0.9 折 | 1M tokens | 通用能力最强 | 复杂架构设计、多文件重构 |
| `gpt-5.4` | GPT 福利 0.9 折 | 1M tokens | 性价比之选 | 日常编码、bug 修复 |

```bash
# 通过 --model 指定模型
codex --model gpt-5.5 "复杂任务"
codex --model gpt-5.4 "日常任务"
```
...
```bash
codex --model gpt-5.4 "在当前目录创建一个 Express.js REST API 项目，包含用户 CRUD，使用 SQLite，包含错误处理和输入验证"
```

代码审查：

```bash
codex --model gpt-5.5 "审查 src/ 目录下的所有代码，找出潜在的性能问题和安全漏洞"
```

编写测试：

```bash
codex --model gpt-5.4 "为 src/utils/ 下的工具函数编写完整的单元测试，使用 Jest 框架"
```
...
# 确认拼写：gpt-5.5（正确）、GPT-5.5（大小写错误）、gpt55（缺少短横线）
```
