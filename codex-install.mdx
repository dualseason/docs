...
| 模型 | 渠道 | 上下文 | 特点 | 适用场景 |
| --- | --- | --- | --- | --- |
| `gpt-4.1` | GPT 福利 0.9 折 | 1M tokens | 通用能力最强 | 复杂架构设计、多文件重构 |
| `gpt-4.1-mini` | GPT 福利 0.9 折 | 1M tokens | 性价比之王 | 日常编码、bug 修复 |
| `gpt-4.1-nano` | GPT 福利 0.9 折 | 1M tokens | 速度最快 | 简单补全、格式化 |
| `o4-mini` | 官 Key 4.9 折 | 200K tokens | 推理+性价比 | 中等复杂度推理任务 |
| `codex-mini` | 待定 | 待定 | 代码专精 | 轻量代码任务 |

```bash
# 通过 --model 指定模型
codex --model gpt-4.1 "你的任务"
codex --model gpt-4.1-mini "你的任务"
codex --model o4-mini "你的推理任务"
```
...
```bash
codex --model gpt-4.1-mini "在当前目录创建一个 Express.js REST API 项目，包含用户 CRUD，使用 SQLite，包含错误处理和输入验证"
```

代码审查：

```bash
codex --model gpt-4.1 "审查 src/ 目录下的所有代码，找出潜在的性能问题和安全漏洞"
```

编写测试：

```bash
codex --model gpt-4.1-mini "为 src/utils/ 下的工具函数编写完整的单元测试，使用 Jest 框架"
```
...
# 确认拼写：gpt-4.1（正确）、GPT-4.1（大小写错误）、gpt4.1（缺少短横线）
```
...
# 指定模型启动
codex --model gpt-4.1 "任务"
codex --model gpt-4.1-mini "任务"
codex --model o4-mini "任务"
